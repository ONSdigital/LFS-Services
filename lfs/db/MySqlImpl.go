...
func (s MySQL) UnpersistDataset(tableName string) (dataset.Dataset, error) {
	...
	req := s.DB.Collection("columns").Find().Where("table_name = '" + tableName + "'").OrderBy("column_number")
	var column Column
	for req.Next(&column) {
	...
	s.log.Info("data unpersisted in %s", a)
	return d, nil
}
...
func (s MySQL) PersistDataset(d dataset.Dataset) error {
	...
	a := time.Now().Sub(startTime)
	s.log.Info("data persisted in %s", a)
